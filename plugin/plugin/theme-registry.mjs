...
const v3Descriptor = async (folder, { vault, bundled = false }) => {
  ...
  for (const dirname of bundled ? [] : Object.values(THEME_PACKAGE_DIRECTORIES)) {
    requiredPackageFolder(vault, folder.path, dirname);
  }
  const visualSpecFiles = (bundled ? [] : THEME_VISUAL_SPEC_FILES).map((filename) =>
    requiredPackageFile(
      vault,
      folder.path,
  ...
  const manifest = parseThemeManifest(await vault.cachedRead(manifestFile), {
    directoryName: bundled ? '' : folder.name,
  });
  ...
  if (manifest.showcaseImage) {
    const filePath = themePackageFilePath(folder.path, manifest.showcaseImage, 'showcase_image');
    const file = getFile(vault, filePath);
    if (file) showcaseImage = { file, filePath, url: vault.getResourcePath(file) };
    else problems.push(`${manifest.name}：showcase_image 文件不存在：${manifest.showcaseImage}`);
  }
  ...
  return {
    theme: {
      ...
      previewImage,
      showcaseImage,
      showcaseStatus: manifest.showcaseStatus,
      source: bundled ? 'bundled' : 'vault',
      assets,
      components,
      fonts,
 ...
 */
export async function discoverVaultThemes({ vault, metadataCache, themeFolder, bundled = false }) {
  ...
  for (const entry of entries) {
    if (!bundled && isFile(entry) && isThemeFile(entry.name) && entry.parent?.path === folderPath) {
      themes.push(v1Descriptor(entry, metadataCache));
      continue;
    }
    if (!isFolder(entry) || entry.parent?.path !== folderPath) continue;
    try {
      const loaded = await v3Descriptor(entry, { vault, bundled });
