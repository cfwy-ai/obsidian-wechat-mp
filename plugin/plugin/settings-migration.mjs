...
export function migrateLegacyThemeSettings(settings) {
  ...
  // Existing installations explicitly chose a Vault folder. Preserve that
  // authoring workflow; a fresh install opens the bundled collection.
  if (!['bundled', 'vault'].includes(settings?.themeSource)) {
    migratedSettings.themeSource = cleanVaultFolder(settings?.themeFolder ?? '')
      ? 'vault'
      : 'bundled';
    migrated = true;
  }
