...
export class WechatPreviewView extends ItemView {
  constructor(leaf, plugin) {
    ...
    this.headingImageRuntime = new HeadingImageRuntime({ vault: plugin.themeResources });
  ...
  async loadThemeState(context) {
    const discovered = await this.plugin.themeResources.discover(this.plugin.settings);
    ...
    const content = await loadVaultThemeContent(selected, {
      vault: this.plugin.themeResources,
      context,
    });
    return {
      themes: discovered.themes,
  ...
  resolveImageRecordFile(image, article) {
    if (image?.filePath) {
      const exact = this.plugin.themeResources.getAbstractFileByPath(image.filePath);
      return exact instanceof TFile || this.plugin.themeResources.ownsFile(exact) ? exact : null;
    }
  ...
  async copyCurrent() {
    ...
    try {
      ...
      const result = await copyRenderedArticle({
        ...
        readBinary: (file) => this.plugin.themeResources.readBinary(file),
      ...
      if (!selected || !css) {
        ...
        this.renderMessage(
          selected
            ...
            : this.plugin.settings.themeSource === 'bundled' ? '内置模板无法读取，请重新安装完整插件包。' : `没有在「${this.plugin.settings.themeFolder}」找到可用风格。`,
        );
        this.setBusy(false);
        return;
      }
