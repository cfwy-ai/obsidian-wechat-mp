 ...
 */
/**
 * 四字目录名是「中文主题目录」的作者约定，用于让菜单在 manifest 省略 name 时仍有显示名。
 * 发布形态的目录名是稳定 theme_id（如 dune-echo），不适用该约定；此时 manifest 的
 * name 必填，显示名不再依赖目录名。
 */
export function parseThemeManifest(source, { directoryName = '', requireAuthoringDocs = true } = {}) {
  let data;
  try {
  ...
  const directoryDisplayName = directoryName && requireAuthoringDocs
    ? validateThemeDirectoryName(directoryName)
    : '';
